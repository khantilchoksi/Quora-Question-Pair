...
def siamese_lstm(layer_cnt, sentence_len, embedding_len, units, name):
    ...
    sharedLSTM = LSTM(units, return_sequences=True, name='{}_Layer_{}'.format(name, 0))
    enc_left = sharedLSTM(left)
    enc_right = sharedLSTM(right)
    encoded = Subtract(name='{}_Difference'.format(name))([enc_left, enc_right])
    for i in range(layer_cnt - 1):
        lstm = LSTM(units, return_sequences=True, name='{}_Layer_{}'.format(name, i + 1))
        encoded = lstm(encoded)
    lstm = LSTM(units, name='{}_Layer_{}'.format(name, layer_cnt))
    encoded = lstm(encoded)
    predictions = Dense(1, activation='sigmoid', name='{}_prediction'.format(name))(encoded)
    model = Model(inputs=[left, right], outputs=predictions, name=name)
    return model
...
def train(indexes, layer_cnt, units, x_train, x_valid, batch_size, name):
    key = name
    ...
    if os.path.exists(os.path.join('models', '{}.h5'.format(key))):
        ...
        model = siamese_lstm(layer_cnt=layer_cnt,
                             sentence_len=20,
                             embedding_len=300,
                             units=units,
                             name=key)
    ...
    if e[key]['trained'] == 'n':
        ...
        tensorboard = TensorBoard(log_dir=os.path.join('logs', '{}_log'.format(key)),
                                  ...
                                  embeddings_layer_names=['{}_Layer_{}'.format(key, x) for x in range(layer_cnt + 1)])
...
def main(layer_cnt, units, name):
    ...
    if not os.path.exists('histories'):
        os.mkdir('histories')
    train(indexes, layer_cnt, units, X_train, X_valid, 128, name)
